...
#Última atualização (17/08): atualização dos parâmetros (suporte e confiança) de ambos algoritmos para geração de novas regras.

#IMPORTANTE: a importação da base e a geração dos arquivos Excel com as regras de associação não funcionarão devido a diferença nos caminhos dos arquivos.
...
df = pd.read_csv(r"C:\Users\thiag\OneDrive\Área de Trabalho\Faculdade\TCC\test.csv")
...
def codigo_apriori(df):   
    ...
    rules.to_excel(r"C:\Users\thiag\OneDrive\Área de Trabalho\Faculdade\TCC\regrasAPRIORI.xlsx", index=False)
...
def eclat(df):
    ...
    rules_eclat.to_excel(
        r"C:\Users\thiag\OneDrive\Área de Trabalho\Faculdade\TCC\regrasECLAT.xlsx",
        index=False
    )
